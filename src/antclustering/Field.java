...
//**********************************************************************************************//
    ...
    //***初期配置を行う***//
    public void Grandset(){
        Random rnd = new Random();
        state = new int[MAX_size][MAX_size];
        ant = new ant[MAX_ant];
        ...
        while(object_size<MAX_object||ant_size<MAX_ant){
            ...
            if(object_kind!=0){
                //オブジェクトの配置
                if(state[object_y][object_x]==0 && object_kind<MAX_kind+1 && object_size<MAX_object){
                    state[object_y][object_x] = object_kind;
                    object_size++;
                //蟻の配置
                }else if(state[object_y][object_x]<ANT && object_kind == ANT && ant_size<MAX_ant){
...
//                pa = rnd2.nextDouble();
                ...
                if(pick(an)){
                    ...
                    if(pickObject(an,i)){
                        //蟻がオブジェクトを持ち上げる
                        antClone[an].State = stateClone[y][x]-ANT;
                        stateClone[y][x] = ANT;
                    }else
                        antClone[an].time++;
                }
                //降ろせる状態か
                    ...
                    if(dropObject(an,i)){
                        //蟻がオブジェクトを下す
                        stateClone[y][x] = antClone[an].State+ANT;
                        antClone[an].State = 0;
                        antClone[an].time = 0;
                    }else
                        antClone[an].time ++;
...
//**********************************************************************************************//
    ...
    public static boolean pick(int an){
        ...
        return ant[an].State==0&&state[ant[an].Location.y][ant[an].Location.x]>ANT;
    }
    //***オブジェクトを降ろせるか***//
...
//**********************************************************************************************//    
    ...
    public static double Neight(int an){
        ...
        if(count==0)return 0;        
        return (count+anotherCount) / objectCount;
    }
    //***周囲8マスの物体の数***//
...
//**********************************************************************************************//
    ...
    public void Move(int an,int oldx,int oldy){
        ...
        while(count<30){
            ...
            if((x>0&&x<MAX_size)&&(y>0&&y<MAX_size)&&state[y][x]<ANT){
                //以前の位置から蟻を削除                   
                state[oldy][oldx] = state[oldy][oldx]-ANT;
                //蟻の移動
                ant[an].Location.x = x;
                ant[an].Location.y = y;
                //移動後の位置に蟻を追加
                state[y][x] = state[y][x]+ANT;
                //ループ強制抜け
                ant[an].oldX = x;
                ant[an].oldY = y;
                break;
